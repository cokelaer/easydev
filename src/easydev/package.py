...
metainfo = {
    ...
    'url' : ["http://packages.python.org/%(name)s/"],
...
class PackageBuilder(object):
    ...
    """
    def __init__(self, name, share=True):
        """.. rubric:: Constructor
    ...
    def init(self, force=False):
        ...
        if os.path.isdir(self.name):
            self.logging.warning("Directory %s already exists." % self.name)
            self.logging.warning("You are about to delete this directory %s. " % self.name)
            if force==True:
                res = "y"
            else:
    ...
    def create_setup(self):
        ...
        filename = self.name + os.sep + "setup.py"
	f = open(filename, "w")
        f.write(setup_template % self.metainfo)
        f.close()

        filename = self.name + os.sep + "setup.cfg"
	f = open(filename, "w")
        f.write("# a setup configuration")
        f.write(setup_cfg_template % {'name':self.name})
        f.close()

    def create_readme(self):
        """Creates a README file"""
        self.logging.info("Creating a README file")
        filename = self.name + os.sep + "README.txt"
	f = open(filename, "w")
